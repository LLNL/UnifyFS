...
// UnifyCR keys
const char* key_runstate           = "unifycr.runstate";
...
#ifndef UNIFYCR_MAX_KV_KEYLEN
# define UNIFYCR_MAX_KV_KEYLEN 256
#endif

#ifndef UNIFYCR_MAX_KV_VALLEN
# define UNIFYCR_MAX_KV_VALLEN 4096
#endif
...
static void unifycr_pmi2_errstr(int rc)
{
    switch (rc) {
    case PMI2_SUCCESS:
...
// initialize PMI2
static int unifycr_pmi2_init(void)
{
...
// finalize PMI2
static int unifycr_pmi2_fini(void)
{
    int rc;
...
static int unifycr_pmi2_lookup(const char* key,
                               char** oval)
{
    int rc, vallen;
...
static int unifycr_pmi2_publish(const char* key,
                                const char* val)
{
    int rc;
    char pmi2_key[PMI2_MAX_KEYLEN] = {0};
...
# define PMIX_MAX_VALLEN UNIFYCR_MAX_KV_VALLEN
#endif

// initialize PMIx
static int unifycr_pmix_init(void)
{
    int rc;
...
// finalize PMIx
static int unifycr_pmix_fini(void)
{
    int rc;
    size_t ninfo;
    pmix_info_t* info;
...
// publish a key-value pair
static int unifycr_pmix_publish(const char* key,
                                const char* val)
{
    int rc;
    size_t ninfo;
...
static char localfs_kvdir[UNIFYCR_MAX_FILENAME];
static char sharedfs_kvdir[UNIFYCR_MAX_FILENAME];
static char sharedfs_rank_kvdir[UNIFYCR_MAX_FILENAME];
static int have_sharedfs_kvstore; // = 0

static int unifycr_fskv_init(unifycr_cfg_t* cfg)
{
    int rc, err;
    struct stat s;
    ...
    if ((UNIFYCR_SERVER == cfg->ptype) && (NULL != cfg->sharedfs_dir)) {
    ...
    kv_max_keylen = UNIFYCR_MAX_KV_KEYLEN;
    kv_max_vallen = UNIFYCR_MAX_KV_VALLEN;
    return (int)UNIFYCR_SUCCESS;
}

static int unifycr_fskv_fini(void)
{
    int rc;
    struct stat s;
    struct dirent* de;
    ...
    if (rc == 0) {
        ...
        char kvfile[UNIFYCR_MAX_FILENAME];
    ...
    if (have_sharedfs_kvstore) {
        ...
        if (rc == 0) {
            ...
            char rank_kvfile[UNIFYCR_MAX_FILENAME];
...
// lookup a key-value pair
static int unifycr_fskv_lookup_local(const char* key,
                                     char** oval)
{
    FILE* kvf;
    char kvfile[UNIFYCR_MAX_FILENAME];
...
static int unifycr_fskv_lookup_remote(int rank,
                                      const char* key,
                                      char** oval)
{
    FILE* kvf;
...
// publish a key-value pair
static int unifycr_fskv_publish_local(const char* key,
                                      const char* val)
{
    FILE* kvf;
    char kvfile[UNIFYCR_MAX_FILENAME];
...
static int unifycr_fskv_publish_remote(const char* key,
                                       const char* val)
{
    FILE* kvf;
    char rank_kvfile[UNIFYCR_MAX_FILENAME];
...
#if defined(USE_PMIX)
        rc = unifycr_pmix_init();
        if (rc != (int)UNIFYCR_SUCCESS) {
            return rc;
        }
#elif defined(USE_PMI2)
        rc = unifycr_pmi2_init();
        if (rc != (int)UNIFYCR_SUCCESS) {
            return rc;
        }
#else
...
int unifycr_keyval_fini(void)
{
    int rc;
    if (kv_initialized) {
        rc = unifycr_fskv_fini();
        if (rc != (int)UNIFYCR_SUCCESS) {
            return rc;
        }

#if defined(USE_PMIX)
        rc = unifycr_pmix_fini();
        if (rc != (int)UNIFYCR_SUCCESS) {
            return rc;
        }
#elif defined(USE_PMI2)
        rc = unifycr_pmi2_fini();
        if (rc != (int)UNIFYCR_SUCCESS) {
            return rc;
        }
#endif
                                ...
                                char** oval)
{
    ...
    if (!kv_initialized) {
        rc = unifycr_keyval_init(NULL, NULL, NULL);
        if (rc != (int)UNIFYCR_SUCCESS) {
            return rc;
        }
    }
    ...
    // do the lookup
    rc = unifycr_fskv_lookup_local(key, oval);
    if (rc != (int)UNIFYCR_SUCCESS) {
                                 ...
                                 char** oval)
{
    ...
    // do the lookup
#if defined(USE_PMIX)
    rc = unifycr_pmix_lookup(rank_key, oval);
#elif defined(USE_PMI2)
    rc = unifycr_pmi2_lookup(rank_key, oval);
#endif
    if (rc != (int)UNIFYCR_SUCCESS) {
        rc = unifycr_fskv_lookup_remote(rank, key, oval);
    }
    if (rc != (int)UNIFYCR_SUCCESS) {
                                 ...
                                 const char* val)
{
    ...
    // publish it
    rc = unifycr_fskv_publish_local(key, val);
    if (rc != (int)UNIFYCR_SUCCESS) {
                                  ...
                                  const char* val)
{
    ...
    // publish it
 #if defined(USE_PMIX)
    rc = unifycr_pmix_publish(rank_key, val);
#elif defined(USE_PMI2)
    rc = unifycr_pmi2_publish(rank_key, val);
#endif
    if (rc != (int)UNIFYCR_SUCCESS) {
        rc = unifycr_fskv_publish_remote(key, val);
    }
