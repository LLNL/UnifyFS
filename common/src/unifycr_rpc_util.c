...
void rpc_publish_local_server_addr(const char* addr)
{
    ...
    unifycr_keyval_publish_local(key_unifycrd_margo_shm, addr);
...
void rpc_publish_remote_server_addr(const char* addr)
{
    ...
    // publish client-server margo address
    unifycr_keyval_publish_remote(key_unifycrd_margo_svr, addr);
}
...
char* rpc_lookup_local_server_addr(void)
{
    ...
    if (0 == unifycr_keyval_lookup_local(key_unifycrd_margo_shm, &valstr)) {
        addr = strdup(valstr);
        free(valstr);
    }
...
char* rpc_lookup_remote_server_addr(int srv_rank)
{
    ...
    if (0 == unifycr_keyval_lookup_remote(srv_rank, key_unifycrd_margo_svr,
                                          &valstr)) {
        addr = strdup(valstr);
        free(valstr);
    }
