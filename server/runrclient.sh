...
UNIFYCR_DIR=../UnifyCR_Client/tests
...
ulimit -c unlimited
srun --drop-caches --nodelist=${nlist} --distribution=block --ntasks-per-node=${procs_per_node} -n ${nprocs} -N ${nnodes} ${UNIFYCR_DIR}/test_listread -b ${BLK_SZ} -s ${SEG_CNT} -t ${TRAN_SZ} -f /tmp/abc -p ${PAT} -u ${TO_UNMOUNT} 2>&1|tee rclient_${nprocs}.log
