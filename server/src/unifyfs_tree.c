...
static unifyfs_state_truncate_t* get_state_truncate(int32_t tag)
{
    // TODO: lookup tag-->state entry in tag2state map, return state
    void* value = int2void_find(&glb_tag2state, tag);
    if (value == NULL) {
    ...
    return (unifyfs_state_truncate_t*)value;
}

static int rpc_invoke_truncate_request(int rank, unifyfs_state_truncate_t* st)
{
    int rc = (int)UNIFYFS_SUCCESS;
...
static int rpc_invoke_truncate_response(int rank, unifyfs_state_truncate_t* st)
{
    int rc = (int)UNIFYFS_SUCCESS;
...
unifyfs_state_truncate_t* state_truncate_alloc(
    ...
    int32_t tag)
{
    unifyfs_state_truncate_t* st = (unifyfs_state_truncate_t*)
        malloc(sizeof(unifyfs_state_truncate_t));
    st->root          = root;
    st->gfid          = gfid;
    st->parent_tag    = ptag;
    st->tag           = tag;
...
{
    if (pst != NULL) {
        unifyfs_state_truncate_t* st = *pst;
...
static void truncate_response_forward(unifyfs_state_truncate_t* st)
{
    ...
    if (st->num_responses == child_count) {
        /* truncate the file */
        int ret = gfid2ext_tree_truncate(&glb_gfid2ext, st->gfid, st->length);

        /* send result to parent if we have one */
        if (parent != -1) {
            printf("%d: BUCKEYES truncate(len=%llu)=%d\n",
                   glb_pmi_rank, (unsigned long long) st->length, ret);
            fflush(stdout);
            ...
            printf("BUCKEYES truncate(len=%llu)=%d, all complete err=%d\n",
                (unsigned long long) st->length, ret, st->err);
            fflush(stdout);
...
void truncate_request_forward(unifyfs_state_truncate_t* st)
{
    printf("%d: BUCKEYES request_forward (truncate: len=%llu)\n",
           glb_pmi_rank, (unsigned long long) st->length);
    fflush(stdout);
...
static void truncate_request_rpc(hg_handle_t handle)
{
    ...
    /* allocate a new structure to track state of this truncate operation,
     * we assign an integer tag to this structure which we pass to any process
     * that will later send us a response, that process will include this tag
     * in its response, and we use the tag value on that incoming message to
     * lookup the structure using a tag2state map */
    ...
    unifyfs_state_truncate_t* st = state_truncate_alloc(root, gfid, length,
                                                        ptag, tag);
    int2void_add(&glb_tag2state, st->tag, (void*)st);
...
static void truncate_response_rpc(hg_handle_t handle)
{
    ...
    /* lookup state structure corresponding to this incoming rpc */
    unifyfs_state_truncate_t* st = get_state_truncate(tag);
