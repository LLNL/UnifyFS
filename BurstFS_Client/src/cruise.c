...
#ifdef CRUISE_GOTCHA
#define NFUNCS 37 
struct gotcha_binding_t wrap_cruise[] = {
    //{ "stat", CRUISE_WRAP(stat), &CRUISE_REAL(stat)}, 
    { "read", CRUISE_WRAP(read), &CRUISE_REAL(read) },
    { "write", CRUISE_WRAP(write), &CRUISE_REAL(write) },
    { "open" , CRUISE_WRAP(open),  &CRUISE_REAL(open)  },
    { "access", CRUISE_WRAP(access), &CRUISE_REAL(access)},
    { "mkdir", CRUISE_WRAP(mkdir), &CRUISE_REAL(mkdir) },
    { "rmdir", CRUISE_WRAP(rmdir), &CRUISE_REAL(rmdir) },
    { "unlink", CRUISE_WRAP(unlink), &CRUISE_REAL(unlink) },
    { "remove", CRUISE_WRAP(remove), &CRUISE_REAL(remove) },
    { "rename", CRUISE_WRAP(rename), &CRUISE_REAL(rename) },
    { "truncate", CRUISE_WRAP(truncate), &CRUISE_REAL(truncate) },
    { "lseek", CRUISE_WRAP(lseek), &CRUISE_REAL(lseek) },
    { "__lxstat", CRUISE_WRAP(__lxstat), &CRUISE_REAL(__lxstat) },
    { "__xstat64", CRUISE_WRAP(__xstat64), &CRUISE_REAL(__xstat64) },
    { "__xstat", CRUISE_WRAP(__xstat), &CRUISE_REAL(__xstat) },
    { "__xstat64", CRUISE_WRAP(__xstat64), &CRUISE_REAL(__xstat64) },
    { "creat", CRUISE_WRAP(creat), &CRUISE_REAL(creat) },
    { "creat64", CRUISE_WRAP(creat64), &CRUISE_REAL(creat64) },
    { "open64", CRUISE_WRAP(open64), &CRUISE_REAL(open64) },
    { "readv", CRUISE_WRAP(readv), &CRUISE_REAL(readv) },
    { "writev", CRUISE_WRAP(writev), &CRUISE_REAL(writev) },
    { "pread", CRUISE_WRAP(pread), &CRUISE_REAL(pread) },
    { "pread64", CRUISE_WRAP(pread64), &CRUISE_REAL(pread64) },
    { "pwrite", CRUISE_WRAP(pwrite), &CRUISE_REAL(pwrite) },
    { "pwrite64", CRUISE_WRAP(pwrite64), &CRUISE_REAL(pwrite64) },
    { "posix_fadvise", CRUISE_WRAP(posix_fadvise), &CRUISE_REAL(posix_fadvise) },
    { "lseek64", CRUISE_WRAP(lseek64), &CRUISE_REAL(lseek64) },
    { "ftruncate", CRUISE_WRAP(ftruncate), &CRUISE_REAL(ftruncate) },
    { "fsync", CRUISE_WRAP(fsync), &CRUISE_REAL(fsync) },
    { "fdatasync", CRUISE_WRAP(fdatasync), &CRUISE_REAL(fdatasync) },
    { "flock", CRUISE_WRAP(flock), &CRUISE_REAL(flock) },
    { "mmap", CRUISE_WRAP(mmap), &CRUISE_REAL(mmap) },
    { "mmap64", CRUISE_WRAP(mmap64), &CRUISE_REAL(mmap64) },
    //{ "munmap", CRUISE_WRAP(munmap), &CRUISE_REAL(munmap) },
    { "msync", CRUISE_WRAP(msync), &CRUISE_REAL(msync) },
    { "__fxstat", CRUISE_WRAP(__fxstat), &CRUISE_REAL(__fxstat) },
    { "__fxstat64", CRUISE_WRAP(__fxstat64), &CRUISE_REAL(__fxstat64) },
    { "close", CRUISE_WRAP(close), &CRUISE_REAL(close) },
    /* stdio functions */
    {"fopen", CRUISE_WRAP(fopen), &CRUISE_REAL(fopen) }
};
#endif
//#endif
...
static int cruise_init(int rank)
{
    if (! cruise_initialized) {
        ...
        result = gotcha_wrap(wrap_cruise, NFUNCS, "cruise");
