                                    ...
                                    size_t filesize)
{
    ...
    for (unsigned i = 0; i < NUM_TEST_FILES; i++) {
    ...
    *     mountpoint is one
    * (4) Get the metadata for each gfid - verifying file size as appropriate
    * TODO: What else can we verify??
    */
    ...
    if ((NULL != databuf) && (NULL != readbuf)) {
        ...
        for (unsigned file_num = 0; file_num < NUM_TEST_FILES; file_num++) {
        ...
        for (unsigned i = 0; i < num_gfids; i++) {
            ...
            for (unsigned j = 0; j < NUM_TEST_FILES; j++) {
